...
@TRANSFORMS.register_module()
class LoadPoseFromFile(BaseTransform):
    def __init__(self,
    ...
    @staticmethod
    def _parse_poses(filename, calibration):
    ...
    def _load_file(self, pose_file_path, calib_file_path):
        calibration = self._parse_calibration(calib_file_path)
        poses = self._parse_poses(pose_file_path, calibration)
        return poses

    def _load_pose(self, results: dict) -> List:
        ...
        Returns:
            dict: The dict containing loaded pose annotations.
        """

        if self.dataset == 'semantickitti':
            lidar_path = results['lidar_points']['lidar_path']
            # seq = lidar_path.split('/')[-3]
            frame = int(lidar_path.split('/')[-1].split('.')[0])

            file_elements = lidar_path.split('/')[:-2]
            pose_file_path = '/'.join(file_elements) + '/poses.txt'
            calib_file_path = '/'.join(file_elements) + '/calib.txt'
            poses = self._load_file(pose_file_path, calib_file_path)
        else:
            raise NotImplementedError
        return poses[frame]

    def transform(self, results: dict) -> dict:
        poses = self._load_pose(results)
        results['lidar_poses'] = poses
        return results

@TRANSFORMS.register_module()
class LoadMetaTest(BaseTransform):
    """Map original semantic class to valid category ids.
    ...
    def transform(self, results: dict) -> dict:
        ...
        if 'eval_ann_info' in results:
            ...
            mask_name = '/'.join(mask_path.split('/')[-3:])
            results['eval_ann_info']['mask_name'] = mask_name
        return results
