...
class MeanStudentHook(Hook):
    ...
    def copy_teacher(self, model: nn.Module) -> None:
        """Compute the moving average of the parameters using exponential
        moving average."""
        if self.skip_buffers:
            for (src_name, src_parm), (dst_name, dst_parm) in zip(
                    model.teacher.named_parameters(),
                    model.student.named_parameters()):
                dst_parm.data.mul_(0).add_(src_parm.data, alpha=1.0)

        else:
            for (src_parm,
                 dst_parm) in zip(model.teacher.state_dict().values(),
                                  model.student.state_dict().values()):
                # exclude num_tracking
                if dst_parm.dtype.is_floating_point:
                    dst_parm.data.mul_(0.0).add_(
                        src_parm.data, alpha=1.0)
