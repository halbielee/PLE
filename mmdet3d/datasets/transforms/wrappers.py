...
@TRANSFORMS.register_module()
class MultiBranch3DUnlabeled(BaseTransform):
    r"""Multiple branch pipeline wrapper.
    ...
    def transform(self, results: list) -> dict:
        """Transform function to apply transforms sequentially.
        ...
        """
        multi_results = {}
        for i, (branch, pipeline) in enumerate(self.branch_pipelines.items()):
            branch_results = pipeline(copy.deepcopy(results[i]))
            # If one branch pipeline returns None,
            # it will sample another data from dataset.
            if branch_results is None:
                return None
