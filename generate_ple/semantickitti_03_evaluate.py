...
class IoUEvaluator:
    """
    A class to evaluate Intersection over Union (IoU) for semantic segmentation tasks.
    Attributes:
    -----------
    n_classes : int
        Number of classes in the dataset.
    ignore : np.ndarray
        Array of class indices to ignore during evaluation.
    include : np.ndarray
        Array of class indices to include during evaluation.
    conf_matrix : np.ndarray
        Confusion matrix to accumulate predictions and ground truths.
    Methods:
    --------
    __init__(n_classes, ignore=None):
        Initializes the IoUEvaluator with the number of classes and classes to ignore.
    num_classes():
        Returns the number of classes.
    reset():
        Resets the confusion matrix to zeros.
    add_batch(x_pred, x_gt):
        Adds a batch of predictions and ground truths to the confusion matrix.
    get_stats():
        Computes true positives, false positives, and false negatives from the confusion matrix.
    get_iou():
        Computes the mean IoU and IoU per class.
    get_acc():
...
def parse_argument():
    parser = argparse.ArgumentParser("./semantickitti_03_evaluate.py")
    parser.add_argument(
        "--gt",
        type=str,
    ...
    parser.add_argument(
        "--pred", type=str, default="R0.5_PLE", help="Path to predicted labels"
    )
    parser.add_argument(
        "--output", type=str, default=None, help="Path to output result"
    )
    parser.add_argument(
        "--config", type=str, default="semantic-kitti.yaml", help="Path to config file"
    )
...
if __name__ == "__main__":
    ...
    with open(args.output, "w") as f:
        sys.stdout = f
        print("SemanticKITTI evaluation")
