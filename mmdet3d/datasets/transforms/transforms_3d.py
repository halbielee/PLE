...
@TRANSFORMS.register_module()
class FrameMove(BaseTransform):
    def __init__(self,
                 pre_transform: Optional[Sequence[dict]],
                 transforms: List,
                 move_frame_range: List[float] = (-3, 3),
                 ):
        self.pre_transform = Compose(pre_transform)
        self.move_frame_range = move_frame_range
        self.transforms = [Compose(transforms) for transforms in transforms]

    @staticmethod
    def get_nearby_idx(src_pts, src_pose, dst_pts, dst_pose):

        # move src_pts to dst_pts
        diff = np.matmul(np.linalg.inv(dst_pose), src_pose)
        transformed_src_pts = np.matmul(diff, src_pts.T).T

        coord1 = dst_pts[:, :3]
        coord2 = transformed_src_pts[:, :3]

        kdtree = KDTree(coord2)
        dist, idx_src2dst = kdtree.query(coord1, k=1)
        return idx_src2dst

    def transform(self, input_dict: dict) -> dict:
        # load nearby points
        dataset = input_dict['dataset']
        cur_index = input_dict['sample_idx']
        sequence_index = input_dict['lidar_path'].split('/')[-2]
        # get index of other point cloud

        min_index = max(0, cur_index + self.move_frame_range[0])
        max_index = min(len(dataset), cur_index + self.move_frame_range[1] + 1)

        is_nearby = False
        nearby_results = None
        # get index of other point cloud
        while not is_nearby:
            index = np.random.randint(min_index, max_index)
            nearby_results = dataset.get_data_info(index)
            nearby_sequence_index = nearby_results['lidar_path'].split('/')[-2]
            if nearby_sequence_index == sequence_index:
                is_nearby = True

        if self.pre_transform is not None:
            nearby_results.update({'dataset': dataset})
            nearby_results = self.pre_transform(nearby_results)
            nearby_results.pop('dataset')
        # apply transformation

        src_lidar_poses = input_dict['lidar_poses']
        dst_lidar_poses = nearby_results['lidar_poses']

        idx_src2dst = self.get_nearby_idx(
            nearby_results['points'].tensor.numpy(), dst_lidar_poses,
            input_dict['points'].tensor.numpy(), src_lidar_poses,
                                          )
        idx_dst2src = self.get_nearby_idx(
            input_dict['points'].tensor.numpy(), src_lidar_poses,
            nearby_results['points'].tensor.numpy(), dst_lidar_poses,
                                          )
        input_dict['frame_consistent_idx'] = idx_src2dst
        nearby_results['frame_consistent_idx'] = idx_dst2src

        for t in self.transforms:
            input_dict = t(input_dict)
            nearby_results = t(nearby_results)

        # print(input_dict.keys())
        # print(input_dict['sample_id'])
        # print(input_dict['sample_idx'])
        # print(input_dict['lidar_path'])
        # print('lidar_points', input_dict['lidar_points'])
        return [input_dict, nearby_results]


@TRANSFORMS.register_module()
class FrustumDrop(BaseTransform):
    def __init__(self,
                 drop_ratio: float = 0.5,
                 remain_range: float = 0.2):

        self.drop_ratio = drop_ratio
        self.remain_range = remain_range
        min_angle = random.uniform(0, 1)
        max_angle = min_angle + remain_range
        if max_angle > 1.0:
            self.remain_angles = [(min_angle, 1.0), (0.0, max_angle - 1.0)]
        else:
            self.remain_angles = [(min_angle, max_angle)]

    def transform(self, input_dict: dict) -> dict:
        if random.uniform(0, 1) < self.drop_ratio:
            points = input_dict['points']
            if 'pts_semantic_mask' in input_dict:
                pts_semantic_mask = input_dict['pts_semantic_mask']

            yaw = -np.arctan2(points.coord[:, 1], points.coord[:, 0])
            proj_x = 0.5 * (yaw / np.pi + 1.0)

            # drop points in angle range
            out_points = []
            if 'pts_semantic_mask' in input_dict:
                out_pts_semantic_mask = []
            for drop_angle in self.remain_angles:
                idx = (proj_x >= drop_angle[0]) & (proj_x <= drop_angle[1])
                out_points.append(points[idx])
                if 'pts_semantic_mask' in input_dict:
                    out_pts_semantic_mask.append(pts_semantic_mask[idx.numpy()])
            out_points = points.cat(out_points)
    ...
    def __repr__(self) -> str:
        """str: Return a string that describes the module."""
        repr_str = self.__class__.__name__
        repr_str += f'(remain_angle={self.remain_angles}, '
        repr_str += f'remain_range={self.remain_range}, '
        repr_str += f'drop_ratio={self.drop_ratio}'
        return repr_str


@TRANSFORMS.register_module()
class Interpolate3DPoints(BaseTransform):
    def __init__(self,
                 resize_prob=0.5,
                 resize_ratio=(1.0, 1.5),
                 height=64,
                 fov_up=3.0,
                 fov_down=-25.0,):
    ...
    def transform(self, input_dict: dict) -> dict:
        ...
        if random.uniform(0, 1) < self.resize_prob:
            resize_ratio = np.random.uniform(self.resize_ratio[0], self.resize_ratio[1])
            points = input_dict['points'].tensor.numpy()
            # get_angle sort by value
            azimuth = -np.arctan2(points[:, 1], points[:, 0])

            # sort by 'azimuth' direction
            ...
            proj_y = np.minimum(self.height - 1, proj_y)
            proj_y = np.maximum(0, proj_y).astype(np.int32)

            src_tensor = torch.from_numpy(points)
            src_tensor = torch.permute(src_tensor, (1, 0))
            out_points = []

            if 'pts_semantic_mask' in input_dict:
                pts_semantic_mask = input_dict['pts_semantic_mask']
                pts_semantic_mask = pts_semantic_mask[sort_indices]
                src_label = torch.from_numpy(pts_semantic_mask)[None, :]
                out_pts_semantic_mask = []

            for h in range(self.height):
                line_idx = np.where(proj_y == h)[0]
                if len(line_idx) == 0:
                    continue

                output_size = int(len(line_idx) * resize_ratio)
                line_tensor = src_tensor[:, line_idx]

                # interpolate points
                dst_tensor = F.interpolate(line_tensor.unsqueeze(0),
                                           size=output_size, mode='linear')
                dst_tensor = torch.squeeze(dst_tensor, 0)
                dst_tensor = torch.permute(dst_tensor, (1, 0))
                dst_tensor = dst_tensor.numpy()

                new_points = input_dict['points'].new_point(dst_tensor)
                out_points.append(new_points)

                # interpolate labels
                if 'pts_semantic_mask' in input_dict:
                    line_label = src_label[:, line_idx].to(torch.uint8)
                    dst_label = F.interpolate(line_label.unsqueeze(0),
                                                size=output_size, mode='nearest')
                    dst_label = torch.squeeze(dst_label, 0).permute(1, 0)
                    dst_label = dst_label.numpy()
                    out_pts_semantic_mask.append(dst_label)

            out_points = input_dict['points'].cat(out_points)
            input_dict['points'] = out_points
            if 'pts_semantic_mask' in input_dict:
                out_pts_semantic_mask = np.concatenate(out_pts_semantic_mask, axis=0)
                input_dict['pts_semantic_mask'] = out_pts_semantic_mask.astype(np.int64)
        return input_dict

    def __repr__(self) -> str:
        """str: Return a string that describes the module."""
        repr_str = self.__class__.__name__
        repr_str += f'(resize_prob={self.resize_prob}, '
        repr_str += f'(resize_ratio={self.resize_ratio}, '
...
@TRANSFORMS.register_module()
class NormalizePoints(BaseTransform):
    def __init__(self,
                 points_mean=(10.88, 0.23, -1.04, 0.21),
                 points_std=(11.47, 6.91, 0.86, 0.16)):
        self.points_mean = np.array(points_mean)[None, :]
        self.points_std = np.array(points_std)[None, :]

    def transform(self, input_dict: dict) -> dict:
        points = input_dict['points'].tensor.numpy()
        points = (points - self.points_mean) / self.points_std
        points = input_dict['points'].new_point(points)
        input_dict['points'] = points
        return input_dict
    ...
    def __repr__(self) -> str:
        """str: Return a string that describes the module."""
        repr_str = self.__class__.__name__
        repr_str += f'(points_mean={self.points_mean}, '
        repr_str += f'(points_std={self.points_std}'
        return repr_str
